...
class RequestHeaders {
  ...
  function checkCorsPolicy($allowedMethod){
    $allowedOrigins = [
     ...
     'http://localhost:5173',
     'http://127.0.0.1:5173',
      strval(substr($_SERVER['HTTP_HOST'], 4))
    ];
    ...
    //   if (isset($_SERVER['HTTP_ORIGIN']) && in_array($_SERVER['HTTP_ORIGIN'], $allowedOrigins)) {
       ...
       header("Access-Control-Allow-Headers: Origin, Content-Type, Route, route, AuthToken, authToken, Authtoken, authtoken");
  ...
  function checkAllHeaders() {
    ...
    foreach ($headers as $header => $value) {
      if($header=="AuthToken" || $header=="authToken" || $header=="Authtoken" || $header=="authtoken"){
        $this->Authorization = $value;
        $this->UserIP = $this->getClientIP();
      }else if($header=="Route" || $header=="route"){
        $this->Route = $value;
