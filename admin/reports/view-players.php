...
if($f_status != "") $where .= " AND tbl_account_status = '$f_status'";
...
$sql = "SELECT * FROM tblusersdata $where ORDER BY id DESC LIMIT $offset, $content";
$total_count = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM tblusersdata $where"))['total'];
...
<head>
    ...
    <style>
        ...
        .btn-filter-submit {
            width: 100%; height: 36px; background: linear-gradient(135deg, #06b6d4, #3b82f6);
            border: none; border-radius: 8px; color: #fff; font-weight: 800; font-size: 11px;
        ...
        .tag-active { background: rgba(16, 185, 129, 0.1); color: #10b981; border: 1px solid rgba(16, 185, 129, 0.2); }
        .tag-banned { background: rgba(244, 63, 94, 0.1); color: #f43f5e; border: 1px solid rgba(244, 63, 94, 0.2); }
    ...
    <div class="admin-main-content hide-native-scrollbar">
        <div class="dash-header">
            <div class="dash-header-left">
                ...
                <h1 class="dash-title">Players Analysis</h1>
            </div>
            <div class="dash-header-right">
                ...
                <button class="btn-modern btn-outline-modern" onclick="window.location.href='view-players.php'">
        ...
        <form method="GET" class="advanced-filter-card">
            <div class="filter-input-group">
                ...
                <div class="filter-input-wrapper">
                    ...
                    <input type="text" name="f_username" class="filter-inp" value="<?php echo $f_username; ?>" placeholder="Search Username..">
                </div>
            </div>
            <div class="filter-input-group">
                ...
                <div class="filter-input-wrapper">
                    ...
                    <select name="f_status" class="filter-inp">
                        <option value="">All Status</option>
                        <option value="true" <?php if($f_status == "true") echo "selected"; ?>>Active</option>
                        <option value="ban" <?php if($f_status == "ban") echo "selected"; ?>>Banned</option>
                    </select>
                </div>
            </div>
            <div class="filter-input-group">
        ...
        <div class="table-responsive">
            <table class="r-table">
                <thead><tr><th>No</th><th>Unique ID</th><th>User Info (Username)</th><th>Mobile</th><th>Balance</th><th>Status</th><th>Joined</th></tr></thead>
                <tbody>
                    <?php 
                    ...
                    <tr>
                        ...
                        <td class="text-info fw-bold">#<?php echo $row['tbl_uniq_id']; ?></td>
                        <td>
                            <div class="fw-bold"><?php echo $row['tbl_user_name'] ?: 'N/A'; ?></div>
                            <div class="text-dim small" style="font-size: 10px;"><?php echo $row['tbl_full_name']; ?></div>
                        </td>
                        <td><?php echo $row['tbl_mobile_num']; ?></td>
                        <td class="fw-bold text-main">₹<?php echo number_format($row['tbl_balance'], 2); ?></td>
                        <td><span class="tag-pill <?php echo $row['tbl_account_status']=='true'?'tag-active':'tag-banned'; ?>"><?php echo $row['tbl_account_status']=='true'?'Active':'Banned'; ?></span></td>
                        <td class="text-dim"><?php echo $row['tbl_user_joined']; ?></td>
                    </tr>
                    <?php endwhile; ?>
                </tbody>
            </table>
        ...
        <div class="d-flex justify-content-between align-items-center mt-4">
            ...
            <div class="d-flex gap-2">
                ...
                <a href="?page_num=<?php echo $i; ?>&f_username=<?php echo $f_username; ?>&f_status=<?php echo $f_status; ?>&f_date_from=<?php echo $f_date_from; ?>&f_date_to=<?php echo $f_date_to; ?>" class="pag-box <?php echo $i==$page_num?'active':''; ?>"><?php echo $i; ?></a>
                <?php endfor; ?>
            </div>
        </div>
    </div>
...
function exportData(format) {
    ...
    const baseUrl = `export.php?type=players&f_username=${f_username}&f_status=${f_status}&f_date_from=${f_date_from}&f_date_to=${f_date_to}`;

    if (format === 'excel') {
        window.location.href = baseUrl + '&format=excel';
    } else if (format === 'pdf') {
        // Fetch JSON for PDF to get ALL data
        fetch(baseUrl + '&format=json')
            ...
            .then(data => {
                ...
                doc.setFontSize(18);
                doc.text('Players Analysis Report', 14, 15);
                doc.setFontSize(10);
                ...
                doc.autoTable({
                    head: [['No', 'Unique ID', 'Username', 'Mobile', 'Balance', 'Status', 'Joined']],
                    body: tableData,
                    startY: 30,
                    theme: 'striped',
                ...
                doc.save('players_full_report_' + new Date().getTime() + '.pdf');
            });
    }
}
</script>
